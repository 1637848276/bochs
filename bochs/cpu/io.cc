...
enum { IO_IN, IO_OUT, IO_INS, IO_OUTS };

void InOutHook(int thread_index, int itype, Bit16u port, Bit64u memptr, int asize, Bit64u data, int dsize )
{

    char csize = 'E';
    if(dsize == 1)  csize = 'B';
    else if(dsize == 2) csize = 'W';
    else if(dsize == 4) csize = 'D';
    else if(dsize == 8) csize = 'Q';

    if(itype == IO_IN) { // IN

        MSG((thread_index, OUTPUT_BOTH, "##in(%c): port = %04x  data = %llx", csize, port, data));

    } else if (itype == IO_OUT) { //OUT

        MSG((thread_index, OUTPUT_BOTH, "##out(%c): port = %04x data = %llx", csize, port, data));

    } else if (itype == IO_INS) { // INS

        MSG((thread_index, OUTPUT_BOTH, "##ins%c%d: port = %04x data = %llx memptr(%d) = %llx",  csize, asize*8, port, data, csize, memptr));

    } else if (itype == IO_OUTS) { // OUTS

        MSG((thread_index, OUTPUT_BOTH, "##outs%c%d: port = %04x data = %llx memptr(%d) = %llx", asize*8, port, data, csize, memptr));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSB16_YbDX(bxInstruction_c *i)
{
 ...
 // MSG((thread_index, OUTPUT_BOTH, "insb16:  dx = %x di = %x", DX, DI));
  InOutHook(thread_index, IO_INS, DX, (Bit64u)DI, 2, (Bit64u)value8, 1);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSB32_YbDX(bxInstruction_c *i)
{
 ...
 // MSG((thread_index, OUTPUT_BOTH, "insb32:  dx = %x edi = %x", DX, EDI));
    InOutHook(thread_index, IO_INS, DX, (Bit64u)EDI, 4, (Bit64u)value8, 1);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSB64_YbDX(bxInstruction_c *i)
{
  ...
  value8 = BX_INP(DX, 1);
   ...
   // MSG((thread_index, OUTPUT_BOTH, "insb64:  dx = %x di = %llx", DX, RDI));
    InOutHook(thread_index, IO_INS, DX, (Bit64u)RDI, 8, (Bit64u)value8, 1);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSW16_YwDX(bxInstruction_c *i)
{
 ...
 //  MSG((thread_index, OUTPUT_BOTH, "insw16:  dx = %x di = %x", DX, DI));
    InOutHook(thread_index, IO_INS, DX, (Bit64u)DI, 2, (Bit64u)value16, 2);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSW32_YwDX(bxInstruction_c *i)
{
...
#endif
  {
   ...
   // MSG((thread_index, OUTPUT_BOTH, "insw32:  dx = %x edi = %x", DX, EDI));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSW64_YwDX(bxInstruction_c *i)
{
  ...
  //MSG((thread_index, OUTPUT_BOTH, "insw64:  dx = %x rdi = %llx", DX, RDI));
  InOutHook(thread_index, IO_INS, DX, (Bit64u)RDI, 8, (Bit64u)value16, 2);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSD16_YdDX(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_INS, DX, (Bit64u)DI, 2, (Bit64u)value32, 4);


  //MSG((thread_index, OUTPUT_BOTH, "insd32:  dx = %x di = %x", DX, DI));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSD32_YdDX(bxInstruction_c *i)
{
  ...
  //MSG((thread_index, OUTPUT_BOTH, "insd32:  dx = %x edi = %x", DX, EDI));
   InOutHook(thread_index, IO_INS, DX, (Bit64u)EDI, 4, (Bit64u)value32, 4);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::INSD64_YdDX(bxInstruction_c *i)
{
  ...
  //MSG((thread_index, OUTPUT_BOTH, "insd64:  dx = %x rdi = %llx", DX, RDI));
   InOutHook(thread_index, IO_INS, DX, (Bit64u)RDI, 8, (Bit64u)value32, 4);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSB16_DXXb(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUTS, DX, (Bit64u)SI, 2, (Bit64u)value8, 1);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSB32_DXXb(bxInstruction_c *i)
{
  ...
  BX_OUTP(DX, value8, 1);
  InOutHook(thread_index, IO_OUTS, DX, (Bit64u)ESI, 4, (Bit64u)value8, 1);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSB64_DXXb(bxInstruction_c *i)
{
  ...
  BX_OUTP(DX, value8, 1);
 InOutHook(thread_index, IO_OUTS, DX, (Bit64u)RSI, 8, (Bit64u)value8, 1);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSW16_DXXw(bxInstruction_c *i)
{
  ...
  BX_OUTP(DX, value16, 2);
   ...
   InOutHook(thread_index, IO_OUTS, DX, (Bit64u)SI, 2, (Bit64u)value16, 2);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSW32_DXXw(bxInstruction_c *i)
{
...
#endif
  {
    value16 = read_virtual_word(i->seg(), esi);
    BX_OUTP(DX, value16, 2);
    InOutHook(thread_index, IO_OUTS, DX, (Bit64u)ESI, 4, (Bit64u)value16, 2);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSW64_DXXw(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUTS, DX, (Bit64u)RSI, 8, (Bit64u)value16, 2);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSD16_DXXd(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUTS, DX, (Bit64u)SI, 2, (Bit64u)value32, 4);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSD32_DXXd(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUTS, DX, (Bit64u)ESI, 4, (Bit64u)value32, 4);
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::OUTSD64_DXXd(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUTS, DX, (Bit64u)RSI, 8, (Bit64u)value32, 4);
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::IN_ALIb(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_IN, port, (Bit64u)SI, 2, (Bit64u)AL, 1);
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::IN_AXIb(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_IN, port, (Bit64u)SI, 2, (Bit64u)AX, 2);
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::IN_EAXIb(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_IN, port, (Bit64u)SI, 2, (Bit64u)RAX, 4);
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::OUT_IbAL(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUT, port, (Bit64u)SI, 2, (Bit64u)AL, 1);
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::OUT_IbAX(bxInstruction_c *i)
{
  ...
  BX_OUTP(port, AX, 2);
  InOutHook(thread_index, IO_OUT, port, (Bit64u)SI, 2, (Bit64u)AX, 2);
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::OUT_IbEAX(bxInstruction_c *i)
{
  ...
  InOutHook(thread_index, IO_OUT, port, (Bit64u)SI, 2, (Bit64u)EAX, 4);
