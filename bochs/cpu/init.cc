...
#endif
{
  ...
  //tj
  haeapi = HaeAPI::getInstance(NULL);
  OnFetch = NULL;
  OnMemRead = NULL;
  OnMemWrite = NULL;
  OnTEBRead = NULL;
  ...
  ExceptionList = (Bit64u)-1;
  TopLevelException = 0;
  ExceptionVecHandler = 0;

  sleep_cnt = 0;
  hSleepDone = CreateEvent(NULL, FALSE, FALSE, NULL);
...
BX_CPU_C::~BX_CPU_C()
{
  ...
  delete haeapi;
  CloseHandle(hSleepDone);

  BX_INSTR_EXIT(BX_CPU_ID);
  BX_DEBUG(("Exit."));
}

void BX_CPU_C::reset(unsigned source, Bit64u imagebase, Bit64u start, Bit64u stacktop)
{
  unsigned n;
...
//BX_WRITE_32BIT_REGZ(BX_32BIT_REG_EDX, get_cpu_version_information());
  ...
  BX_CPU_THIS_PTR eflags = 0x202; // Bit1 is always set
...
/* initialise MSR registers to defaults */
  ...
  _TEB = GSReadQword(0x30);
  _PEB = GSReadQword(0x60);
  Bit64u *p = (Bit64u*)(_TEB+0x60);
  _TEBend = _TEB + 0x1818;
  _PEBend = _PEB + 0x380;

 //MSG((thread_index, OUTPUT_BOTH, "_TEB: %llx _PEB: %llx (==%llx)", _TEB, _PEB, *p));

  RIP = start;
  RSP = stacktop;
  RDX = RIP;
  R9 = RIP;
  if(_g_flags & TARGET_IS_DLL) {
   // prepare call stack (call dll_main)
    RCX = imagebase; //hinstDLL
    RDX = 1;         //DLL_PROCESS_ATTACH
    R8 = 0;          //lpReserved
    RSP -= 32;
  }
