/////////////////////////////////////////////////////////////////////////
// $Id: ctrl_xfer64.cc 12600 2015-01-11 20:50:26Z sshwarts $
/////////////////////////////////////////////////////////////////////////
//
...
BX_CPP_INLINE void BX_CPP_AttrRegparmN(1) BX_CPU_C::branch_near64(bxInstruction_c *i)
{
  ...
  if (! IsCanonical(new_RIP)) {
    BX_ERROR(("branch_near64: canonical RIP violation: newRIP = %llx", new_RIP));
...
//  if(thread_index == 1) MSG((0, OUTPUT_BOTH, "rsp: %llx", RSP));
  Bit64u return_RIP = stack_read_qword(RSP);
//  if(thread_index == 1)  MSG((0, OUTPUT_BOTH, "return_RIP: %llx", return_RIP));

#if 1
  if (! IsCanonical(return_RIP)) {
    BX_ERROR(("%s: canonical RIP violation: RIP = %llx @%llx", i->getIaOpcodeNameShort(), return_RIP, RIP-i->ilen()));
    exception(BX_GP_EXCEPTION, 0);
  }
#endif
  ...
  if(_Debug) {
      MSG((thread_index, OUTPUT_DEBUG, "ret: rip = %llx", RIP));
  }
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::RETfar64_Iw(bxInstruction_c *i)
{
  ...
  MSG((thread_index, OUTPUT_BOTH, "RETfar called prev-rip = %llx rip = %llx", RIP- i->ilen(), RIP));
...
#if 1
  if (! IsCanonical(new_RIP)) {
    BX_ERROR(("%s: canonical RIP violation: %llx", i->getIaOpcodeNameShort(), new_RIP));
    exception(BX_GP_EXCEPTION, 0);
  }
#endif
...
BX_INSF_TYPE BX_CPP_AttrRegparmN(1) BX_CPU_C::CALL_EqR(bxInstruction_c *i)
{
  ...
  if (! IsCanonical(new_RIP))
  {
    BX_ERROR(("%s: canonical RIP violation: RIP = %llx", i->getIaOpcodeNameShort(), new_RIP));
    exception(BX_GP_EXCEPTION, 0);
  }
#endif
